...
#
# THE PROBLEM
#   SPICE's clipboard travels in three hops:
#     SPICE client (UTM) <-virtio-> spice-vdagentd <-unix socket-> agent
#   The daemon talks to the host; the session agent only talks to the
#   daemon. The OFFICIAL agent hands the clipboard off to X11 (vdagent.c:421)
#   and under Hyprland it dies with "cannot open display", so the daemon is
#   left with no one to hand it to.
#
# THE FIX
#   Replace the AGENT, not the daemon: omarchy-arm-vdagent speaks the same
#   protocol to vdagentd and uses wl-copy/wl-paste. And start the daemon
#   with -X, because its "active seat0 session" check fails under Hyprland
#   launched by SDDM and it silently drops the clipboard.
#
# REQUIREMENT
#   In UTM: VM Settings → Sharing → "Share clipboard" enabled, and the VM
#   opened as a window (not just started with utmctl: with no SPICE client
...
for c in python3 wl-copy wl-paste; do
  command -v "$c" >/dev/null 2>&1 && echo "  ✓ $c" || { echo "  ✗ missing $c"; failed=1; }
done
if [ ! -e /dev/virtio-ports/com.redhat.spice.0 ]; then
  echo "  ✗ /dev/virtio-ports/com.redhat.spice.0 doesn't exist"
  echo "    Enable 'Share clipboard' in UTM and power-cycle the VM."
  failed=1
else
  echo "  ✓ SPICE channel present"
fi
pacman -Q spice-vdagent >/dev/null 2>&1 && echo "  ✓ spice-vdagent installed" \
  || { echo "  ✗ spice-vdagent missing: sudo pacman -S spice-vdagent"; failed=1; }
[ "$failed" -ne 0 ] && { echo; echo "Fix the above and try again."; exit 1; }
...
if [ -f /usr/share/omarchy-arm-vdagent ]; then
  ...
  tmp=$(mktemp)
  curl -fsSL "$RAW" -o "$tmp" || { echo "  ✗ couldn't download it"; rm -f "$tmp"; exit 1; }
  python3 -c "import ast,sys;ast.parse(open(sys.argv[1]).read())" "$tmp" \
    || { echo "  ✗ what was downloaded isn't valid Python"; rm -f "$tmp"; exit 1; }
...
echo
echo "==> daemon with -X"
# Without -X, vdagentd can't find "seat0's active session" under Hyprland
# and drops the clipboard without any error.
...
# The official agent doesn't come only from systemd. First-release images
...
# spice-vdagent.service doesn't cover it, and the pkill above only kills it
# for the current session. Leave this alone and the clipboard works until
# you reboot: on the way back there are two agents and vdagentd cuts both,
...
if [ -f "$AUTO" ] && grep -q 'spice-vdagent' "$AUTO"; then
  ...
  if grep -q '^[[:space:]]*hl\.exec_cmd(.*spice-vdagent' "$AUTO"; then
    echo "  ✗ couldn't disable it in $AUTO; comment it out by hand:"
    grep -n 'spice-vdagent' "$AUTO"
    exit 1
  fi
  hyprctl reload >/dev/null 2>&1 || true
  echo "  autostart.lua: official agent disabled (backup at $AUTO.bak.*)"
else
  echo "  autostart.lua: doesn't launch the official agent"
fi

sleep 1
# On Arch both units are "static" (no [Install] section): `enable` does
# nothing and `is-enabled` never says "enabled". Socket activation is what
# brings them up on every boot. So all that's needed here is making sure
# they're not masked and the socket is alive.
...
case "$(systemctl is-enabled spice-vdagentd.socket 2>/dev/null)" in
  masked) echo "  ✗ spice-vdagentd.socket is masked; the clipboard won't come back after reboot:"
...
if systemctl --user is-active --quiet omarchy-arm-vdagent; then
  echo "  ✓ the clipboard should now work both ways."
  echo "    Copy something on the Mac and paste it here, or the other way around. Text only."
else
  echo "  ✗ the agent didn't start:"
