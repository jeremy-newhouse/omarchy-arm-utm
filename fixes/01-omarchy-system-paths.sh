#!/bin/bash
# Replicates at the system's fixed paths what the pacman package
# `omarchy` does (which only exists for x86_64). The quattro branch expects the
# tree at /usr/share/omarchy and the binaries in the system PATH; without that
# OMARCHY_PATH ends up empty, bashrc fails, and Hyprland falls into emergency
# mode from not finding /usr/share/omarchy/default/hypr/bootstrap.lua
#
...
log "2/8 Omarchy binaries in the system PATH"
# The package publishes them as /usr/bin/omarchy-*; we use /usr/local/bin to
# avoid invading pacman's territory. It comes before /usr/bin and is in sudo's
...
# HISTORICAL: this used to install a development public key to allow SSH
# access during the build. No key is distributed: the final image ships
...
log "8/8 theme as user, with the correct environment now"
