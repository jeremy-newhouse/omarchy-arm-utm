#!/bin/bash
# Replicates in the system's fixed paths what the pacman package `omarchy`
# does (which only exists for x86_64). The quattro branch expects the tree at
# /usr/share/omarchy and the binaries in the system PATH; without that
# OMARCHY_PATH stays empty, bashrc fails, and Hyprland falls back to
# emergency mode for not finding /usr/share/omarchy/default/hypr/bootstrap.lua
#
# Runs as ROOT inside the chroot (without sudo).
set -uo pipefail
USR=gabriel
...
# avoid invading pacman's territory. It comes before /usr/bin and is in
...
log "8/8 theme as user, now with the correct environment"
