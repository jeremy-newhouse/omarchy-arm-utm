...
# chroot, without repartitioning or downloading anything again. For iterating
# after a one-off failure.
set -eu
PROV=/media/prov
...
# utilities: this stage does NOT depend on having network.
for m in btrfs vfat fat nls_cp437 nls_iso8859-1 nls_utf8 crc32c-generic xxhash_generic; do
  modprobe "$m" 2>/dev/null || true
done
grep -qw btrfs /proc/filesystems || { echo "!! the live kernel doesn't support btrfs"; exit 1; }
...
# The working directory must not stay inside the system: otherwise every
# repair script from every pass would pile up in there.
